...
var registerNF = func(registerCtx context.Context, newPlmnConfig []models.PlmnId) {
	...
	for {
		...
		case <-time.After(interval):
			...
			if err != nil {
				logger.NrfRegistrationLog.Errorln("register UDR instance to NRF failed. Will retry.", err.Error())
				interval = retryTime
				continue
			}
			logger.NrfRegistrationLog.Infoln("register UDR instance to NRF with updated profile succeeded")
...
func heartbeatNF(plmnConfig []models.PlmnId) {
	...
	if shouldRegister(problemDetails, err) {
		...
		if err != nil {
			logger.NrfRegistrationLog.Errorln("register UDR instance error:", err.Error())
		} else {
			logger.NrfRegistrationLog.Infoln("register UDR instance to NRF with updated profile succeeded")
		}
	} else {
		logger.NrfRegistrationLog.Debugln("UDR update NF instance (heartbeat) succeeded")
	}
...
func shouldRegister(problemDetails *models.ProblemDetails, err error) bool {
	if problemDetails != nil {
		logger.NrfRegistrationLog.Warnln("UDR update NF instance (heartbeat) problem details:", problemDetails)
		return true
	}
	if err != nil {
		logger.NrfRegistrationLog.Warnln("UDR update NF instance (heartbeat) error:", err.Error())
		return true
	}
	return false
}
